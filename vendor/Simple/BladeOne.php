...
class BladeOne
{
     ...
     * @return string
     */
    protected function compileError(string $key): string
    {
//        $key = $this->stripParentheses($key);
/*        return $this->phpTag . '$message = call_user_func($this->errorCallBack,' . $key . '); if ($message): ?>';*/
//                $key = $this->stripParentheses($key);
        $message = error($this->stripQuotes($this->stripParentheses($key)));
        return $this->phpTag . '$message="'.$message.'"; if ($message): ?>';
    }
